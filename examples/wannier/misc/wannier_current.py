...
from schnetpack.atomistic.ewald_traditional import *
import torch
...
def read_data(path):
    ...
    energy_list=[]
    ewald_list=[]
    ewald_reciprocal_list=[]
    force_list=[]
    neigh_mismath_list = []

    lst_dir = [x for x in os.listdir(path) if "." not in x][0:10]
    for dir in lst_dir:
        ...
        if "W64-bulk-HOMO_centers_s1-1_0.xyz" in os.listdir(file_path1):
            ...
            if list(np.unique(wc_neigh)) == [4]:
                ...
                cell_vectors = torch.from_numpy(cell)
                pos_update=np.concatenate((pos_mol,wan_pos))
                positions=torch.from_numpy(pos_update)
                charges_update=np.concatenate((np.where(z_mol == 1, 1, 6),np.full((64,), 8)))
                charges=torch.from_numpy(charges_update)
                print("shape of positions and charges",pos_update.shape,charges_update.shape)
                kmax=12
                alpha=1.0
                ewald_sum_torch, ewald_direct_torch, ewald_reciprocal_torch, ewald_self_torch = compute_ewald_sum(cell_vectors, positions, charges,kmax=kmax,alpha=alpha)
                ...
                print("Ewald details")
                print(ewald_reciprocal_torch)
                print("Total Energy", energy)
                force_list.append(force)
                ewald_list.append(ewald_sum_torch)
                ewald_reciprocal_list.append(ewald_reciprocal_torch)
            else:
